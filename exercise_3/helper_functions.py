...
def transform_points(points: np.ndarray, dx: float, dy: float, scaling: float, rotation: float) -> np.ndarray:
    ...
    """
    ######################################################
    # Add ones to coordinates for transformation to work
    points = np.c_[points, np.ones((points.shape[0], 1))]

    # Apply transformation matrix
    transformation_matrix = np.array([[scaling * np.cos(rotation), -scaling * np.sin(rotation), dx],
                                     [scaling * np.sin(rotation), scaling * np.cos(rotation), dy]])
    transformed_points = np.matmul(points, transformation_matrix.T)
    ######################################################
